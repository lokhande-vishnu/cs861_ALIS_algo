...
def main():
    ...
    alis_avg_test_errors = [0]*n_sample_draws
    rs_avg_test_errors = [0]*n_sample_draws
    for i in range(avg_iter_num):
        alis_i = runALIS(train, test, n_sample_draws)
        alis_avg_test_errors = np.add(alis_avg_test_errors, np.divide(alis_i, avg_iter_num))
        rs_i =  runRS(train, test, n_sample_draws)
        rs_avg_test_errors = np.add(rs_avg_test_errors, np.divide(rs_i, avg_iter_num))
    ...
    plt.ylabel('Average Test Set Error over %d iterations' % (avg_iter_num))
